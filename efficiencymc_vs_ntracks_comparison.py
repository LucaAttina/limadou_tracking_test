...
def plot_efficiency_vs_radius(input_dir, output_dir):
    ...
    with open(dump_path, 'w') as dump:
        ...
        for i, fname in enumerate(files):
            ...
            # Print M2 results
            print(f"M2 Efficiency - Tracks: {n_tracks_per_event} = {100*eff_m2_list[-1]:.2f}%")
            print(f"M2 3-cluster Efficiency - Tracks: {n_tracks_per_event} = {100*eff_m2_3cl_list[-1]:.2f}%")
            print(f"M2 2-cluster Efficiency - Tracks: {n_tracks_per_event} = {100*eff_m2_2cl_list[-1]:.2f}%")
            print(f"\nM2 Fake Rate - Tracks: {n_tracks_per_event} = {100*fake_m2_list[-1]:.2f}%")
            print(f"M2 3-cluster Fake Rate - Tracks: {n_tracks_per_event} = {100*fake_m2_3cl_list[-1]:.2f}%")
            print(f"M2 2-cluster Fake Rate - Tracks: {n_tracks_per_event} = {100*fake_m2_2cl_list[-1]:.2f}%")
    ...
    create_efficiency_plots(n_tracks, eff_m2_list, eff_m2_3cl_list, eff_m2_2cl_list, err_m2_list, err_m2_3cl_list, err_m2_2cl_list, output_dir, r_val, miny=0.7, maxy=1.05, cl=2)
    create_fake_rate_plots(n_tracks, fake_m2_list, fake_m2_3cl_list, fake_m2_2cl_list, err_fake_m2_list, err_fake_m2_3cl_list, err_fake_m2_2cl_list, output_dir, r_val, miny=0.0, maxy=0.45, cl=3)
...
def create_efficiency_plots(n_tracks, eff_m2_list, eff_m2_3cl_list, eff_m2_2cl_list, err_m2_list, err_m2_3cl_list, err_m2_2cl_list, output_dir, radius, miny, maxy, cl=None):
    """
    ...
    # Create canvas
    c = ROOT.TCanvas(f"c_eff_vs_ntracks_comparison_cl", f"Efficiency vs Number of Tracks - Clusters comparison", 800, 600)
    
    # M2 graph
    gr_eff = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'), 
    ...
    gr_eff.SetTitle(f"Efficiency vs Number of Tracks - radius: {radius} mm - M2;Number of Tracks;Efficiency")
    gr_eff.Draw("AP")

    gr_3cl = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'),
    ...
    gr_2cl = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'),
    ...
    # Save plot
    plot_path = os.path.join(output_dir, f"efficiency_vs_ntracks_r_{r_str}_cl_comparison.pdf")
    c.SaveAs(plot_path)
    print(f"Plot saved to: {plot_path}")


def create_fake_rate_plots(n_tracks, fake_m2_list, fake_m2_3cl_list, fake_m2_2cl_list, err_fake_m2_list, err_fake_m2_3cl_list, err_fake_m2_2cl_list, output_dir, radius, miny, maxy, cl=None):
    """
    ...
    # Create canvas
    c = ROOT.TCanvas(f"c_fake_vs_ntracks_comparison_cl", f"Fake Rate vs Number of Tracks - Clusters comparison", 800, 600)
    
    # M2 graph
    gr_fake = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'), 
    ...
    gr_fake.SetTitle(f"Fake Rate vs Number of Tracks - radius: {radius} mm - M2;Number of Tracks;Fake Rate")
    gr_fake.Draw("AP")
    ...
    # Save plot
    plot_path = os.path.join(output_dir, f"fake_rate_vs_ntracks_r_{r_str}_cl_comparison.pdf")
    c.SaveAs(plot_path)
    print(f"Plot saved to: {plot_path}")
