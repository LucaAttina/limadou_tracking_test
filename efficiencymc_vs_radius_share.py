    ...
    with open(dump_path, 'w') as dump:
        ...
        for i, fname in enumerate(files):
            ...
            if "not_shared" in fname:
                ...
                print(f"M2 Efficiency (3-clusters / not_shared) - Radius: {r_val:.2f} mm = {100*eff_not_sh_3cl_list[-1]:.2f}%")
                print(f"M2 Efficiency (2-clusters / not_shared) - Radius: {r_val:.2f} mm = {100*eff_not_sh_2cl_list[-1]:.2f}%")
                print(f"\nM2 Fake Rate (3-clusters / not_shared) - Radius: {r_val:.2f} mm = {100*fake_not_sh_3cl_list[-1]:.2f}%")
                print(f"M2 Fake Rate (2-clusters / not_shared) - Radius: {r_val:.2f} mm = {100*fake_not_sh_2cl_list[-1]:.2f}%")
            else:
                s_str = "shared"
                ...
                print(f"M2 Efficiency (3-clusters / shared) - Radius: {r_val:.2f} mm = {100*eff_sh_3cl_list[-1]:.2f}%")
                print(f"M2 Efficiency (2-clusters / shared) - Radius: {r_val:.2f} mm = {100*eff_sh_2cl_list[-1]:.2f}%")
                print(f"\nM2 Fake Rate (3-clusters / shared) - Radius: {r_val:.2f} mm = {100*fake_sh_3cl_list[-1]:.2f}%")
                print(f"M2 Fake Rate (2-clusters / shared) - Radius: {r_val:.2f} mm = {100*fake_sh_2cl_list[-1]:.2f}%")
...
def create_efficiency_plots(radii, eff_not_sh_list, err_not_sh_list, eff_sh_list, err_sh_list, output_dir, n_tracks_per_event, miny, maxy, cl=None, s_str=None):
    """
    ...
    # Create canvas
    c = ROOT.TCanvas(f"c_eff_vs_radius_{cl if cl is not None else 'all'}_cl", f"Efficiency vs Radius - {cl if cl is not None else 'All'} Clusters", 800, 600)
    
    # M2 graph
    gr_ns = ROOT.TGraphErrors(n_points, np.array(radii, dtype='float64'), 
    ...
    # Save plot
    plot_path = os.path.join(output_dir, f"efficiency_vs_radius_{n_tracks_per_event}_evs_{cl if cl is not None else 'all'}_cl_share.pdf")
    c.SaveAs(plot_path)
    print(f"Plot saved to: {plot_path}")


def create_fake_rate_plots(radii, fake_not_sh_list, err_fake_not_sh_list, fake_sh_list, err_fake_sh_list, output_dir, n_tracks_per_event, miny, maxy, cl=None, s_str=None):
    """
    ...
    # Create canvas
    c = ROOT.TCanvas(f"c_fake_vs_radius_{cl if cl is not None else 'all'}_cl", f"Fake Rate vs Radius - {cl if cl is not None else 'All'} Clusters", 800, 600)
    
    # M2 graph
    gr_ns_f = ROOT.TGraphErrors(n_points, np.array(radii, dtype='float64'), 
    ...
    #r_str = f"{radius:.2f}".replace('.', 'p')
    # Save plot
    plot_path = os.path.join(output_dir, f"fake_rate_vs_radius_{n_tracks_per_event}_evs_{cl if cl is not None else 'all'}_cl_share.pdf")
    c.SaveAs(plot_path)
    print(f"Plot saved to: {plot_path}")
