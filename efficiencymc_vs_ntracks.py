...
def plot_efficiency_vs_ntracks(input_dir, output_dir):
    ...
    with open(dump_path, 'w') as dump:
        ...
        fake_m1_2cl_list = []
        err_fake_m1_list = []
        err_fake_m1_3cl_list = []
        err_fake_m1_2cl_list = []
        fake_m2_list = []
        fake_m2_3cl_list = []
        fake_m2_2cl_list = []
        err_fake_m2_list = []
        ...
        for i, fname in enumerate(files):
            ...
            eff_m1_list.append(results["efficiency"]["m1"]["tot"])
            eff_m1_3cl_list.append(results["efficiency"]["m1"]["3cl"])
            eff_m1_2cl_list.append(results["efficiency"]["m1"]["2cl"])
            err_m1_list.append(results["efficiency"]["m1"]["tot_err"])
            err_m1_3cl_list.append(results["efficiency"]["m1"]["3cl_err"])
            err_m1_2cl_list.append(results["efficiency"]["m1"]["2cl_err"])
            fake_m1_list.append(results["fake"]["m1"]["tot"])
            fake_m1_3cl_list.append(results["fake"]["m1"]["3cl"])
            fake_m1_2cl_list.append(results["fake"]["m1"]["2cl"])
            err_fake_m1_list.append(results["fake"]["m1"]["tot_err"])
            err_fake_m1_3cl_list.append(results["fake"]["m1"]["3cl_err"])
            err_fake_m1_2cl_list.append(results["fake"]["m1"]["2cl_err"])
            ...
            print(f"M2 Efficiency (3-clusters) - Tracks per event: {n_tracks_per_event} = {100*eff_m2_3cl_list[-1]:.2f}%")
            print(f"M2 Efficiency (2-clusters) - Tracks per event: {n_tracks_per_event} = {100*eff_m2_2cl_list[-1]:.2f}%")
    ...
    # Create plots
    create_efficiency_plots(n_tracks, eff_m1_list, err_m1_list, eff_m2_list, err_m2_list, output_dir, r_val, miny=0.0, maxy=1.2, cl=None, s_str=s_str)
    create_efficiency_plots(n_tracks, eff_m1_3cl_list, err_m1_3cl_list, eff_m2_3cl_list, err_m2_3cl_list, output_dir, r_val, miny=0.0, maxy=1.2, cl=3, s_str=s_str)
    create_efficiency_plots(n_tracks, eff_m1_2cl_list, err_m1_2cl_list, eff_m2_2cl_list, err_m2_2cl_list, output_dir, r_val, miny=0.0, maxy=1.2, cl=2, s_str=s_str)
    create_fake_rate_plots(n_tracks, fake_m1_list, err_fake_m1_list, fake_m2_list, err_fake_m2_list, output_dir, r_val, miny=0.0, maxy=0.25, cl=None, s_str=s_str)
    create_fake_rate_plots(n_tracks, fake_m1_3cl_list, err_fake_m1_3cl_list, fake_m2_3cl_list, err_fake_m2_3cl_list, output_dir, r_val, miny=0.0, maxy=0.05, cl=3, s_str=s_str)
    create_fake_rate_plots(n_tracks, fake_m1_2cl_list, err_fake_m1_2cl_list, fake_m2_2cl_list, err_fake_m2_2cl_list, output_dir, r_val, miny=0.0, maxy=0.5, cl=2, s_str=s_str)


def create_efficiency_plots(n_tracks, eff_m1_list, err_m1_list, eff_m2_list, err_m2_list, output_dir, radius, miny, maxy, cl=None, s_str=None):
    """
    ...
    # M2 graph
    gr_m1 = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'), 
                        np.array(eff_m1_list, dtype='float64'), 
                        np.zeros(n_points), np.array(err_m1_list, dtype='float64'))
    gr_m1.SetMarkerStyle(20)
    gr_m1.SetMarkerColor(ROOT.kBlue)
    gr_m1.SetLineColor(ROOT.kBlue)
    gr_m1.SetMaximum(maxy)
    gr_m1.SetMinimum(miny)
    gr_m1.SetTitle(f"Efficiency vs Number of Tracks - radius: {radius} mm - {cl if cl is not None else 'All'} Clusters ({s_str});Number of Tracks;Efficiency")
    gr_m1.Draw("AP")
    gr_m2 = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'), 
    ...
    # Save plot
    plot_path = os.path.join(output_dir, f"efficiency_vs_ntracks_r_{r_str}_{cl if cl is not None else 'all'}_cl_{s_str}.pdf")
    c.SaveAs(plot_path)
    print(f"Plot saved to: {plot_path}")


def create_fake_rate_plots(n_tracks, fake_m1_list, err_fake_m1_list, fake_m2_list, err_fake_m2_list, output_dir, radius, miny, maxy, cl=None, s_str=None):
    """
    ...
    # M2 graph
    gr_m1 = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'), 
                        np.array(fake_m1_list, dtype='float64'), 
                        np.zeros(n_points), np.array(err_fake_m1_list, dtype='float64'))
    gr_m1.SetMarkerStyle(20)
    gr_m1.SetMarkerColor(ROOT.kBlue)
    gr_m1.SetLineColor(ROOT.kBlue)
    gr_m1.SetMaximum(maxy)
    gr_m1.SetMinimum(miny)
    gr_m1.SetTitle(f"Fake Rate vs Number of Tracks - radius: {radius} mm - {cl if cl is not None else 'All'} Clusters ({s_str});Number of Tracks;Fake Rate")
    gr_m1.Draw("AP")
    gr_m2 = ROOT.TGraphErrors(n_points, np.array(n_tracks, dtype='float64'), 
    ...
    r_str = f"{radius:.2f}".replace('.', 'p')
    # Save plot
    plot_path = os.path.join(output_dir, f"fake_rate_vs_ntracks_r_{r_str}_{cl if cl is not None else 'all'}_cl_{s_str}.pdf")
    c.SaveAs(plot_path)
    print(f"Plot saved to: {plot_path}")
