...
def compute_efficiency(input_file, output_dir, config, plot_flag, theta_angle_threshold=5.0, phi_angle_threshold=5.0):
    ...
    # Open dump file
    dump_path = os.path.join(log_dir, f"efficiency_dump_{n_tracks_per_event}_evs.txt")
...
#                dump.write(f"\n -- Correctly reconstructed tracks: {sum(good_values)}/{n_tracks_per_event} --\n")
    ...
    if plot_flag:
        ...
        for var in ["theta", "phi"]:
            ...
            # Plot fake tracks
            plot_fake_tracks(var, h_fake_ratio_m1, h_fake_ratio_m2, pdf_fake_single, pdf_fake_comp, n_tracks_per_event)
...
def plot_efficiencies(var, h_eff_m1, h_eff_m2, pdf_eff_single, pdf_eff_comp, n_tracks_per_event, ncls=None):
    ...
    leg = ROOT.TLegend(0.78, 0.84, 0.95, 0.95)
    leg.AddEntry(h_eff_m1, "M1", "lep")
    leg.AddEntry(h_eff_m2, "M2", "lep")
...
def plot_fake_tracks(var, h_fake_ratio_m1, h_fake_ratio_m2, pdf_fake_single, pdf_fake_comp, n_tracks_per_event):
    """
    Plot efficiency histograms.
    """
    # Single efficiency plots
    for hist, hname, color in [
        (h_fake_ratio_m1, f"c_fake_ratio_{var}_m1", ROOT.kBlue),
        (h_fake_ratio_m2, f"c_fake_ratio_{var}_m2", ROOT.kRed),
    ]:
        hist.SetStats(0)
        c = ROOT.TCanvas(hname, "", 800, 600)
        hist.SetLineColor(color)
    ...
    # Comparison plot
    c = ROOT.TCanvas(f"c_fake_ratio_{var}_compare", "", 800, 600)
    h_fake_ratio_m1.SetTitle(f"Fake Tracks Ratio vs #{var} - {n_tracks_per_event} tracks per event")
    ...
    h_fake_ratio_m1.SetMaximum(1.15 * ymax)
    h_fake_ratio_m1.Draw("E1")
    h_fake_ratio_m2.Draw("E1 SAME")
    
    leg = ROOT.TLegend(0.78, 0.84, 0.95, 0.95)
    leg.AddEntry(h_fake_ratio_m1, "M1", "lep")
    leg.AddEntry(h_fake_ratio_m2, "M2", "lep")
    leg.SetTextSize(0.04)
    leg.SetFillStyle(1001)
    leg.SetFillColor(ROOT.kWhite)
    leg.Draw()
